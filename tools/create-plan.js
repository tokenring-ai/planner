...
export async function execute({ task, maxSubtasks = 10 }, registry) {
 ...
 const client = modelRegistry.getFirstOnlineClient({ tags: ['chat','reasoning'] } );

 const [json] = await client.generateObject({
  input: [
   {role: 'system', content: 'You are an expert planner. Break the user\'s task into atomic subtasks.'},
   {role: 'user', content: `Task: "${task}". Break this task into clear, atomic subtasks.`}
  ],
  schema: TaskPlan,
 }, registry);
...
export const description = "Breaks a user-provided task into a numbered list of atomic subtasks.";

export const parameters = z.object({
 task: z.string().describe("The high-level task or project to break down."),
 maxSubtasks: z.number().min(1).max(20).default(10).describe("Maximum number of subtasks to generate")
});
